...
app.use(cors());
app.use(express.json({limit: '50mb'})); // لضمان استقبال ملفات كبيرة
...
<head>
    <meta charset="UTF-8">
    <title>نظام الفواتير - النسخة الاحترافية</title>
...
<body>
    <div class="container">
        ...
        <div class="toolbar">
            ...
            <button onclick="document.getElementById('importEx').click()" class="btn btn-import">استيراد إكسل (النسخة النهائية)</button>
        ...
        function importExcel(e) {
            ...
            reader.onload = async (evt) => {
                ...
                if (invoices.length > 0) {
                    ...
                    if (res.ok) {
                        alert('تم استيراد ' + invoices.length + ' فواتير بنجاح!');
                        load();
                    } else {
...
// المسار الجديد للاستيراد الجماعي (Bulk)
app.post('/api/invoices/bulk', (req, res) => {
    const { invoices } = req.body;
    const placeholder = invoices.map(() => "(?, ?, ?, ?, ?, ?, ?, ?)").join(", ");
    const values = [];
    invoices.forEach(inv => {
        values.push(inv.sales_rep, inv.invoice_number, inv.invoice_date, inv.customer_name, inv.delivery_status, inv.driver_name, inv.branch, inv.notes);
    });

    db.run(`INSERT INTO invoices (sales_rep, invoice_number, invoice_date, customer_name, delivery_status, driver_name, branch, notes) VALUES \${placeholder}`, values, function(err) {
